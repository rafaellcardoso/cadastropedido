...
begin
  ...
  begin
    ...
    cdsItens.Fields[3].AsString  := TProdutoFacade.GetInstancia.GetDescricao(ItemPedido.IdProduto);
...
begin
  ...
  begin
    ...
    cds.Fields[2].AsString   := Pedido.Cliente;
    cds.Fields[3].AsCurrency := Pedido.ValorTotal;
    cds.Post;
  end;
end;
...
begin
  Result := ' AND data_emissao BETWEEN  '+ QuotedStr(FormatDateTime('yyyy-MM-dd', DateIni.Date))+ ' AND  ' + QuotedStr(FormatDateTime('yyyy-MM-dd', DateFim.Date));

  if Length(Trim(EdPesquisa.Text)) > 0 then
  begin
    case CBoxPesquisar.ItemIndex of
      0: Result := Result + ' AND numero_pedido = ' + EdPesquisa.Text;
    end;
  end;
end;
